  ...
  const handleOAuthLogin = async (provider: 'google' | 'github') => {
    try {
      if (!import.meta.env.VITE_SUPABASE_URL) {
        alert("⚠️ Supabase n'est pas configuré. Ajoutez vos clés dans .env.local.");
        return;
      }
        ...
        >
          ...
          <div className="space-y-3 relative z-10">
            <button 
              ...
              className="w-full flex items-center justify-center gap-3 bg-white text-black py-3.5 rounded-xl font-medium text-sm transition-transform hover:scale-[1.02] shadow-md"
            >
            ...
            <button 
              ...
              className="w-full flex items-center justify-center gap-3 bg-[#24292F] text-white py-3.5 rounded-xl font-medium text-sm transition-transform hover:scale-[1.02] border border-white/5"
            >
            ...
            <div className="relative py-4">
              ...
              <div className="relative flex justify-center text-xs">
                <span className="bg-[#121214] px-2 text-white/40">Vous n'avez pas vos clés API Supabase ?</span>
              </div>
            </div>
          </div>
        </div>
